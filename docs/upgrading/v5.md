...
## Use the `useSearchParams` hook for query string management

React Router v6 introduces a new react hook API `useSearchParams`. Whilst similar to the `navigate` API, `useSearchParams` works only on query string part of the url. 

The hook returns an array of two objects, `searchParams` (a `URLSearchParams` object containing the urls current query parameters ), and a function that can be used to update them.

```js
// This is a React Router v6 example of useSearchParams
import { useSearchParams } from "react-router-dom";

function App() {
  const [searchParams, setSearchParams] = useSearchParams();

  const updateSearchParam = e => setSearchParam(e)
  return <button onClick={() => updateSearchParam("test")}>Update</button>
}
```

An example of how to retrieve data from the `searchParams` object can be found below.
...
An example of how to update the search parameters using the returned `setSearchParams` function is also provided below.
...
To see the exact API of the new `useSearchParams` hook and its type declaration, check out our [API Reference](../hooks/use-search-params).

For react native developers, see the react native API of the new `useSearchParams` hook and its type declaration, check out our [React Native API Reference](../hooks/use-search-params-rn).
